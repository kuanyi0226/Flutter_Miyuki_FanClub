...
class Level_y2006 extends World {
  late TiledComponent level;
  final String levelName;
  final Player player;
  late final FollowingSpotlight spotlight1;
  late final FollowingSpotlight spotlight2;
  late final DarkEffect darkEffect;
  List<CollisionBlock> collisionBlocks = [];
  List<String> otherplayers = []; //uid
  late SpriteComponent _background;

  Level_y2006({required this.levelName, required this.player});
  ...
  FutureOr<void> onLoad() async {
    ...
    //spotlights
    spotlight1 = FollowingSpotlight(
        playerSize: player.size,
        source: Vector2(75, -50),
        lightColor: Color.fromARGB(255, 104, 187, 255).withOpacity(0.4))
      ..size = Vector2(100, 100) // Set size to match the shadow size
      ..position = Vector2.zero();
    add(spotlight1);
    spotlight1.priority = 100;
    spotlight2 = FollowingSpotlight(
        playerSize: player.size,
        source: Vector2(625, -50),
        lightColor: Color.fromARGB(255, 104, 187, 255).withOpacity(0.4))
      ..size = Vector2(100, 100) // Set size to match the shadow size
      ..position = Vector2.zero();
    add(spotlight2);
